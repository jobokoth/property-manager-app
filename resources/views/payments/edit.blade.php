                    ...
                    <form method="POST" action="{{ route('payments.update', $payment) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('tenancy_id') is-invalid @enderror" id="tenancy_id" name="tenancy_id">
                                ...
                                @foreach($tenancies as $tenancy)
                                    <option value="{{ $tenancy->id }}" {{ old('tenancy_id', $payment->tenancy_id) == $tenancy->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('payer_user_id') is-invalid @enderror" id="payer_user_id" name="payer_user_id">
                                ...
                                @foreach($payers as $payer)
                                    <option value="{{ $payer->id }}" {{ old('payer_user_id', $payment->payer_user_id) == $payer->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('source') is-invalid @enderror" id="source" name="source" required>
                                <option value="manual" {{ old('source', $payment->source) == 'manual' ? 'selected' : '' }}>Manual Entry</option>
                                <option value="mpesa" {{ old('source', $payment->source) == 'mpesa' ? 'selected' : '' }}>M-Pesa</option>
                            </select>
                            @error('source')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $payment->amount) }}" min="0.01" required>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('paid_at') is-invalid @enderror" id="paid_at" name="paid_at" value="{{ old('paid_at', $payment->paid_at->format('Y-m-d\TH:i')) }}" required>
                            @error('paid_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('reference') is-invalid @enderror" id="reference" name="reference" value="{{ old('reference', $payment->reference) }}">
                            @error('reference')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
